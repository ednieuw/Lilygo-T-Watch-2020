...
//#include <Adafruit_ST7789.h>  // https://github.com/adafruit/Adafruit_ILI9341
...
//--------------------------------------------
// WATCH React on watch.
// Like touch, swipe, auto turn off en turn on
//--------------------------------------------
void WatchOperation(void)
{
 static int minuteSave;
 static int counterToPowOff;
 String formatTime = ttgo->rtc->formatDateTime(PCF_TIMEFORMAT_HMS); 
...
//  int touch=waitDisplayTouch();                             // wait for display touches
/*  
if (touch>0) 
  {    
    counterToPowOff=0;
...
void printBattery()
{
 int battPerc, battPercView, battLoading=0;   
  if (ttgo->power->isVBUSPlug())                // Check if battery is charging    
  {
    battLoading=1;    
  }     
  battPerc=ttgo->power->getBattPercentage();    // Get battary status
...
void EverySecondCheck(void)
{
 ...
 if (msLeap >999)                                                                             // Every second enter the loop
  { 
   ...
   ScreenSecondProcess();                                                                     // Draw seconds in digital and analogue display mode
...
{ 
 ...
 switch(Language)                                                                             //
     {
      case 0: 
      strncpy(Template,"SHETVISOWASOVIJFQPRECIESTIENKPFKWARTTSVOORSOVERAHALFSMIDDERTVIJFATWEESOEEENOXVIERELFTIENKTWAALFBHDRIECNEGENANACHTFZESVZEZEVENSNOENVUQUOTEUURFRTT",145);  
...
//  No Display attached
//--------------------------------------------
//void Displayprint(const char *tekst)  { tekst = tekst;   }                                 // Print to an attached display
//void Displayprintln(const char *tekst){ tekst = tekst;   }
                                      #endif // NODISPLAY
                                      #ifdef ILI9341
//--------------------------------------------                                                //
...
// tft.fillScreen(ConvertRGB32toRGB16(C_ORANGE));                                             // Convert colors to RGB16, except C_BLACK (=0x000000)
 ttgo->tft->setTextColor(C_BLACK, ConvertRGB32toRGB16(C_ORANGE));                                    // Convert colors to RGB16, except C_BLACK (=0x000000)
 ttgo->tft->setCursor(0, ttgo->tft->height()/3);
 ttgo->tft->setTextSize(2);
 ttgo->tft->println("Starting");
}
//--------------------------------------------                                                //
...
void DrawMinuteHand(int mm, int ss, uint8_t smx, uint8_t smy, uint32_t Kleur)
{
 ...
 ttgo->tft->drawLine(omx - 2 * xcorr, omy - 2 * ycorr, smx - 2 * xcorr, smy - 2 * ycorr, Kleur);                                
 ttgo->tft->drawLine(omx - xcorr,     omy - ycorr,     smx - xcorr,     smy - ycorr,     Kleur);  
 ttgo->tft->drawLine(omx,             omy,             smx,             smy,             Kleur); 
 ttgo->tft->drawLine(omx + xcorr,     omy + ycorr,     smx + xcorr,     smy + ycorr,     Kleur);
 ttgo->tft->drawLine(omx + 2 * xcorr, omy + 2 * ycorr, smx + 2 * xcorr, smy + 2 * ycorr, Kleur); 
}
//--------------------------------------------
...
// sprintf(sptext,"   UUR: hh:%d mm:%d ohx:%d ohy:%d ",hh,mm,ohx,ohy ); Tekstprintln(sptext);
 ...
 if(hh>1  && hh <5) {xcorr = 1; ycorr = 0;} 
 if(hh>4  && hh <8) {xcorr = 0; ycorr = 1;}    
 if(hh>7  && hh <11) {xcorr = 1; ycorr = 0;} 
 if(hh>10 || hh <2)  {xcorr = 0; ycorr = 1;}       
// ttgo->tft->fillRect(ohx - 2 * xcorr, ohy - 2 * ycorr, smx + 2 * xcorr, smy+2 * xcorr, Kleur); 

 ttgo->tft->drawLine(ohx - 2 * xcorr, ohy - 2 * ycorr, smx - 2 * ycorr, smy -2 * ycorr, Kleur); 
 ttgo->tft->drawLine(ohx - xcorr,     ohy - ycorr,     smx - ycorr,     smy - ycorr,    Kleur); 
 ttgo->tft->drawLine(ohx,             ohy,             smx,             smy,            Kleur);
 ttgo->tft->drawLine(ohx + xcorr,     ohy + ycorr,     smx + xcorr,     smy + xcorr,    Kleur); 
 ttgo->tft->drawLine(ohx + 2 * xcorr, ohy + 2 * ycorr, smx + 2 * xcorr, smy + 2 * xcorr,Kleur);     
}
//--------------------------------------------                                                //
